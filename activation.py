...
class Activation:
    ...
    def eventActivation(self, listening_pin, activation_pin, activation_duration=0):        # Event based activation. Direct GPIO connections only
    ...
    def boardEvent(self, listening_pin, activation_duration=0):            # Event based activation. For use with PWM board only
        if self.board == None:
            ...
            _listening_pin = Button(listening_pin, pull_up = False)
            _activation_channel = self.board.channels[self.channel]
