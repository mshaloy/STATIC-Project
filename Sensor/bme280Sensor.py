...
class BME280Sensor(sensor):
    ...
    def __init__(self, name, units="Imperial", sleep_t=1.0, repeat = 0, filter=1, standby=0, temp_oversample=1, pressure_oversample=1, humidity_oversample=1):
        ...
        if filter >= 0 and filter <= 4:
            bme280.filter = filter  # 0 to 4 are valid. Filter coefficient. For noise reduction
        else:
            raise Exception("Invalid filter number. Valid values are 0 to 4.")
        
        if standby >= 0 and standby <= 7:
            bme280.standby_time = standby   # 0 to 7 are valid. Time between readings. 0 is 0.5ms
        else:
            raise Exception("Invalid standby value. Valid values are 1 to 16")
        
        if temp_oversample >= 1 and temp_oversample <= 16:
            bme280.over_sample = temp_oversample # 1 to 16 are valid. 0 disables temperature sensing and is not supported
        else:
        ...
        if pressure_oversample >= 1 and pressure_oversample <= 16:
            bme280.pressure_oversample = pressure_oversample # 1 to 16 are valid. 0 disables pressure sensing and is not supported
        else:
        ...
        if humidity_oversample >= 1 and pressure_oversample <= 16:
            bme280.humidity_oversample = humidity_oversample # 0 to 16 are valid. 0 disables pressure sensing and is not supported
        else:
    ...
    def takeReading(self):
        ...
        reading_data = []
        if self.units == "Imperial" or self.units == "Mixed 2":
            reading_data = [self.bme280.temperature_fahrenheit]
        else:
            reading_data = [self.bme280.temperature_celsius]


        if self.units == "Imperial" or self.units == "Mixed 1":
            reading_data.append(self.bme280.altitude_feet)
        else:
            reading_data.append(self.bme280.altitude_meters)

        reading_data.append(self.bme280.pressure)
        reading_data.append(self.bme280.humidity)
